...
const toPermissionPath = (
  modules: Record<string, Record<string, { VIEW?: string; EDIT?: string; ADD?: string; DELETE?: string }>>
) => {
  ...
  Object.entries(modules).forEach(([moduleKey, moduleValue]) => {
    Object.entries(moduleValue).forEach(([permKey, permValue]) => {
      permissionsWithPath[`/${moduleKey}/${permKey.toLowerCase().replaceAll('_', '-')}`] = Object.values(permValue);
    });
  });
